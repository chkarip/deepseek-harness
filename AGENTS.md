...
- **Never run a test suite unless the user asks** (`test`, `test:coverage`, `vitest`).
- Match evidence to the surface: focused tests for behavior, snapshots for model or user output, `doc-sync` for docs, build/hygiene and built smokes for published paths, real-API e2e for providers.
- Never repeat a passing check for commit or push. CI owns exhaustive coverage and the platform matrix; rehearse locally only for CI diagnosis or an irreducibly repository-wide change.
