...
flowchart LR
  ...
  pkg_session_handoff["session-handoff"]
  svc_sessionHandoff["ctx.sessionHandoff<br/>Cross-session context handoff and answer relay"]
  pkg_api_remotes["api-remotes"]
  pkg_client_ui_panels["client-ui-panels"]
  ...
  pkg_skill_mode["skill-mode"]
  svc_skillMode["ctx.skillMode<br/>Sticky skill mode state"]
  ...
  pkg_session_handoff --> svc_sessionHandoff
  ...
  svc_sessionHandoff --> pkg_api_remotes
  svc_sessionHandoff --> pkg_client_ui_panels
...
| `ctx.sessionHandoff` | `core` | [`session-handoff`](../packages/context/session-handoff) | - | [`api-remotes`](../packages/api/remotes), [`client-ui-panels`](../packages/client/ui-panels) | - | 在会话之间转发已完成的助手回答及其对应的提问，采用标签安全的封装和字节预算裁剪。 |
...
| `ctx.skillMode` | `core` | [`skill-mode`](../packages/skill/skill-mode) | - | - | - | 折叠已记录的 skill/mode 状态，在轮次边界刷新用户选择，把活跃模式技能的正文渲染为系统提示词分节，注册 /mode，并在技能不再符合条件时撤下该模式。 |
