...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.chat.workSummary',
    kind: 'keyed',
    scope: 'session',
    summary: 'The collapsed work group\'s activity line: keyed dispatch on the LAST member Node\'s kind, so each row family names what it was doing ("Read <path>") without the chat view importing tool presentation.',
    doc: 'The collapsed work group\'s activity line: keyed dispatch on the LAST\nmember Node\'s kind, so each row family names what it was doing ("Read\n<path>") without the chat view importing tool presentation. Declared by\nthe chat view entry; a kind with no entry leaves the group showing its\nstep count alone.',
    registerOptions: [
      {
        name: 'key',
        requirement: 'required',
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, tool-call',
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.view\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-conversation ReasoningWorkSummary key \'assistant-step\'',
      'client-ui-tool ToolWorkSummary key \'tool-call\'',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.chat.workSummary\', () => ctx.slots.register(\n      { name: \'conversation.chat.workSummary\', key: \'<one key the owner dispatches>\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation.session.header\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-activity-monitor ActivityHeaderPill id \'activity-monitor-pill\'',
  ...
  },
  {
    ...
    occupants: [
      'client-ui-activity-monitor ActivityMonitorView id \'activity\'',
