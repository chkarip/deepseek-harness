...
export interface IApiClient {
  sessions: {
    ...
    mergeForks(payload: RequestPayload<'session.mergeForks'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'session.mergeForks'>>>
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'session.mergeForks': sessionMergeForksValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    mergeForks: (payload, signal) => this.callUnary('session.mergeForks', payload, signal),
