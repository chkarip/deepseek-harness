...
The package also registers `ToolWorkSummary` for the `tool-call` key of `conversation.chat.workSummary`, the activity line a collapsed work group shows for a Tool call: the same row title and one-line summary the expanded row derives, from the same pure model.
