...
export class FakeApiClient implements IApiClient {
  ...
  onMergeForks: (payload: unknown) => Promise<RpcResponse<{
    merged: { forkSessionId: SessionId; appendedEvents: number }[]
    failed: { forkSessionId: SessionId; reason: string }[]
  }>> =
    () => Promise.resolve(ok({ merged: [], failed: [] }))
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    mergeForks: (payload: unknown) => this.record('session.mergeForks', payload, this.onMergeForks(payload)),
