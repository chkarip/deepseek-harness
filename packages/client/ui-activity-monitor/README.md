...
The plugin registers three surfaces:

1. `conversation.session.header.utilities` — `ActivityHeaderPill`, a compact avatar and throughput badge. Clicking it opens the mascot playground and the telemetry panel.
2. `conversation.input.dock` — `MascotDock`, the companion above the composer. It reacts to the session's state (`idle`, `thinking`, `streaming`, `tool`, `approval`, `error`), carries the coffee and happiness controls, and collapses to a one-line bar.
3. `conversation.view` — `ActivityMonitorView`, a full tab with the throughput sparkline, the turn-execution pipeline, the context and cache gauges, and the skin picker.
...
- **Estimates** — the sparkline, the header badge, and the mascot's "tokens fed" come from `useLiveTelemetry`, which approximates tokens from streamed character counts (the browser never sees provider token accounting). Their copy says so.
